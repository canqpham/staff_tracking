...
# client_socket.connect(('http://127.0.0.1', 8001))
...
while True:
    ...
    # img_encode = cv2.imencode('.ipg',frame)
    # data_encode = np.array(img_encode)
    # str_encode = data_encode.tostring('base64')
    ...
    for encoding in encodings:
        ...
        if True in matches:
            ...
            for i in matchedIdxs:
                nameTemp = data["names"][i]
                counts[nameTemp] = counts.get(nameTemp, 0) + 1
    ...
    x = datetime.datetime.now()
    if int(x.strftime("%H")) == 4 and int(x.strftime("%f")) > 800000:
        # print(x)
        check = []
...
#     set_interval(face_recog, 1)
#
# app = web.Application()
# app.add_routes(routes)
# web.run_app(app)
# # app.on_startup.append(set_interval(face_recog, 1))
# # web.run_app(app)
